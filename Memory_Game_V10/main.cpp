 ...
 * File:   main.cpp
 * Author: Emma Wuysang
 * Created on February 6, 2024, 2:25 AM
 * Purpose: Concentration Card Game V.10
 */
...
//Function Prototypes
int compareGuess(char,char,int,int &, char[], string[], int SIZE=20);
bool shownTable (string[]);
int dsplyVal(int score);
float dsplyVal(float &avg, int score);
void VctrTbl(const vector<char>& VctrTbl, int ROW, int COLS);
...
int main(int argc, char** argv) {
    ...
    while(gameSes=="y"){ // leads to loading screen   
        ...
        chce2="";
        bool tblView = false;
        while((chce1 == 'y' && chce2 != "over") && (chce1 == 'y' && tblView == false)){
            ...
            char shwTbl;
            cout<<endl<<"Would you like to see the table placements fully revealed? (type 'y' to view) \n"
...
int compareGuess(char userG1,char userG2,int moves,int &score, char deck[], string cardNam[], int SIZE){
    ...
    if (deck[temp1]+10==deck[temp2] || deck[temp1]-10==deck[temp2] ){
        ...
        moves += 1;
        fndAll+=1;
        deck[temp1] = -11; // accounts for if you've guessed something or not
        deck[temp2] = -11;
    }
    ...
    if (tempScr>=10){
        ...
        if (endQ=='y'){
            ...
            const int ROW = 5;
            const int COLS = 4;
            vector<char> VctrDsp = {'a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t'};
...
bool shownTable (string cardNam[]){ // option to display table after leaving guessing portion update
    ...
    return true; // you've seen the table!
}
...
int dsplyVal (int score){
    ...
    cout<<"View a simulated scoreboard? (type 's' to preview) \n";
    char scrbd;
    cin>>scrbd;
