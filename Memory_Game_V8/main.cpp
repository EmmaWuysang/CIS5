 ...
 * File:   main.cpp
 * Author: Emma Wuysang
 * Created on January 31, 2024, 2:37 PM
 * Purpose: Concentration Card Game V.8
 */
...
int compareGuess(char userG1,char userG2,int moves,int &score, char deck[], string cardNam[]){ // update allows user to exit at any given 10 + guesses 
    bool complete;
    int SIZE = 20;
    ...
    if (tempScr>10){ // resets the temporary score
        tempScr = 0;
    }
    
    if (tempScr>=10){
        cout<<"Would you like to end the game? (type 'y' to end)"<<endl;
        cin>>endQ;
        if (endQ=='y'){
            return 10;
        }
