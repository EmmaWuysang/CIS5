...
//Function Prototypes
int compareGuess(char,char,int,int &, char[], string[], int SIZE=20); //compares the two guesses, main portion of the game
...
void VctrTbl(const vector<char>& VctrTbl, int ROW, int COLS); // vector table version, redisplayed by user in guessing
...
int main(int argc, char** argv) {
    ...
    int smlBrd[]= {12,10,20,1,5,6,13,7,2,9,11,4,8,14,19,16,17,18,15,3}; // one unsorted array for example 2
    int smlBrd1[]= {20,11,3,13,5,19,17,8,16,14,2,12,4,10,15,9,1,18,6,7}; // one unsorted array for example 1
    ...
    while(gameSes=="y"){ // leads to loading screen   
        ...
        chce2="";
        bool tblView = false;
        while((chce1 == 'y' && chce2 != "over") && (chce1 == 'y' && tblView == false)){ // if the user hasn't declared over, or hasn't viewed the answer table 
            cout<<setw(28)<<"The Concentration Card Board!\n"; // initial game display 
            const int rows = 5; // initialized for two dimensional array 
            const int cols = 4;
            cout<<setw(26)<<"_______________________\n";
            ...
            string cardNam[20] = {newCd1,newCd2,newCd3,newCd4,newCd5,newCd6,newCd7,newCd8,newCd9,newCd10,
                   newCd11,newCd12,newCd13,newCd14,newCd15,newCd16,newCd17,newCd18,newCd19,newCd20}; //holds all the names of the cards in string
            ...
            while(fndAll != 10){ //stops when pair is found
                ...
                fndAll = compareGuess(userG1,userG2,moves,score,deck,cardNam); // fndAll determines if they've found all 10 pairs.
            }
            
            char shwTbl; 
            cout<<endl<<"Would you like to see the table placements fully revealed? (type 'y' to view/else otherwise) \n" // shows table answers, won't allow you to play the game anymore
...
int compareGuess(char userG1,char userG2,int moves,int &score, char deck[], string cardNam[], int SIZE){ // compares the guesses
    ...
    for (int i=0;i<SIZE;i++){ // iterates through the card size, does linear search through array for specific card
    ...
    if (tempScr>=10){
        ...
        if (endQ=='y'){ // if user decides on to end game early
            ...
            const int ROW = 5;
            const int COLS = 4;
            vector<char> VctrDsp = {'a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t'}; // the vector display table

            VctrTbl(VctrDsp, ROW, COLS);
        }
        else{
            cout<<"The game will continue!\n"<<endl;
            cout<<"Tip : Writing down your guesses could come in handy! \n"<<endl; // gives tip to user after 10 guesses, and if they decide to continue playing 
        }
    }
} 
...
void bublSrt(int smlBrd[],int smlSIZE){ // does bubble sort
    ...
    while (check==false){ 
        ...
        for (int i=0;i<smlSIZE-1;i++){
            if (smlBrd[i] > smlBrd[i+1]){ // compares a number, to the one adjacent to i, right one larger, swaps numbers
